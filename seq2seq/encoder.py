...
source_int = [[source_letter_to_int.get(letter, target_letter_to_int['<UNK>'])
...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size, target_sequence_length, max_target_sequence_length, encoder_size, decoder_input):
	#1. Embedding
	target_vocab_size = len(target_letter_to_int)
	decoder_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, decoding_embedding_size]))
	decoder_embed_input = tf.nn.embedding_lookup(decoder_embeddings, decoder_input)

	#2. construct decoder RNN unit
	def get_decoder_cell(rnn_size):
		decoder_cell = tf.contrib.rnn.LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=2))
		return decoder_cell
	cell = tf.contrib.rnn.MultiRNNCell([get_decoder_cell(rnn_size) for _ in range(num_layers)])

	#3. build output full connection
...
def seq2seq_model(input_data, targets, lr, target_sequence_length,
	...
	#send state vector and input to decoder
...
with train_graph.as_default():
	...
	training_logits = tf.identify(training_decoder_output.rnn_output, 'logits')
	predicting_logits = tf.identify(predicting_decoder_output.sample_id, name='predictions')
...
def pad_sentence_batch(sentence_batch, pad_int):
	max_sequence = max(len(sentence) for sentence in sentence_batch)
	return [sentence + [pad_int] * (max_sequence - len(sentence)) for sentence in sentence_batch]
...
def get_batches(targets, sources, batch_size, source_pad_int, target_pad_int):
	for batch_i in range(0, len(sources)//batch_size):
		...
		yield pad_targets_batch, pad_sources_batch, pad_targets_lengths, pad_sources_lengths
...
print('\nSource')
print('  Word id:    {}'.format([i for i in text]))
print('  Input Words: {}'.format(" ".join([source_int_to_letter[i] for i in text])))

print('\nTarget')
print('  Word id:       {}'.format([i for i in answer_logits if i != pad]))
